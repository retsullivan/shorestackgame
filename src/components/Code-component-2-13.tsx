...
export function ThemeSelector({ currentTheme, onThemeChange, unlockedThemes }: ThemeSelectorProps) {
  ...
  return (
    <div className="absolute top-4 right-4 z-50">
      <div className="bg-beach-foam p-3 rounded-lg pixel-border">
        ...
        <div className="grid grid-cols-2 gap-2">
          {themes.map((theme) => (
            <Button
              ...
              className={`w-12 h-12 p-1 border-2 transition-all ${
            ...
            >
              {!unlockedThemes.includes(theme.id) && (
                <div className="w-3 h-3 bg-beach-dark-rock rounded-full"></div>
              )}
            </Button>
          ))}
        </div>
